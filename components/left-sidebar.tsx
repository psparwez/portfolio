...
export default function LeftSidebar({ isMenuOpen }: LeftSidebarProps) {
  return (
    <motion.div
      initial={{ x: '-100%' }}
      animate={{ x: 0 }}
...
const Logo = () => {
  return (
    <Link
      ...
      className='dark:bg-border-color! border-border-color! relative flex aspect-square h-auto w-12.5 shrink-0 items-center justify-center overflow-hidden rounded-2xl border bg-transparent! p-1 transition-all duration-300 hover:scale-95 dark:border-zinc-800!'
    >
      <div className='absolute inset-0 rounded-full'>
        <Image
          src={SITE_CONFIG.profile.avatar}
          alt={`${SITE_CONFIG.profile.name}'s profile avatar`}
          fill
          quality={100}
        ...
        return (
          ...
          >
            <Link
              ...
              className={cn(
                'group relative flex h-min w-min cursor-pointer items-center justify-center rounded-xl border border-transparent p-3.5 transition-colors duration-300',
                isActive
